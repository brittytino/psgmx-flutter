...
// 2. READ SEED
const seedPath = path.join(__dirname, '../complete_user_seed.sql');
let existingData = {};

try {
    const seedContent = fs.readFileSync(seedPath, 'utf8');
    // Regex to match value tuples loosely
    const lines = seedContent.split('\n');
    for (const line of lines) {
         if (line.trim().startsWith("(") && line.includes("@psgtech.ac.in")) {
             // Basic parsing: ('email', 'name', 'reg', 'batch', 'team', 'roles')
             // Split by delimiter ', ' is risky if json contains commas
             // RegExp extraction
             const match = line.match(/^\('([^']+)',\s*'([^']+)',\s*'([^']+)',\s*'([^']+)',\s*'([^']+)',\s*'(\{.*?\})'\),?$/);
             if (match) {
                 const [_, email, name, reg_no, batch, team_id, roles] = match;
                 existingData[reg_no.trim()] = { email, team_id, roles };
             }
         }
    }
} catch (err) {
...
for (const line of lines) {
    ...
    const [regNo, name, batch, dob, leetcode] = parts;
    if (regNo === 'NULL') continue;
    
    let email = `${regNo.toLowerCase()}@psgtech.ac.in`;
    let teamId = 'T00';
    let roles = '{"isStudent": true, "isTeamLeader": false, "isCoordinator": false, "isPlacementRep": false}';
    
    if (existingData[regNo]) {
        const old = existingData[regNo];
        email = old.email;
        teamId = old.team_id;
        roles = old.roles;
    }
    ...
    values.push(`('${email}', '${nameEscaped}', '${regNo}', '${batch}', '${teamId}', ${fmtDate(dob)}, ${fmt(leetcode)}, '${roles}')`);
}
...
fs.writeFileSync(path.join(__dirname, '../complete_user_seed_v2.sql'), sql);
console.log("Done");
